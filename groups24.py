...
SEEDS = [0, 1, 2]          # was seed 0 only, which under-reported the table


def _key(group, seed):
    """Legacy naming: seed 0 has no suffix, so existing results/ablation.json
    and the README table keep working unchanged."""
    return group if seed == 0 else f"{group}_seed{seed}"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        jobs = [(g, sd) for g in ("D4xZ3", "Q8xZ3") for sd in SEEDS]
    results = {}
    for g, sd in jobs:
        P_twin, P_pair, P_ones = build_pair_projector(g)
        model, val, n = train_model(g, seed=sd)
        ...
        results[_key(g, sd)] = row
        print(f"{g} seed{sd}", {k: round(v, 3) for k, v in row.items()}, flush=True)
    # merge, never clobber: a single-seed rerun used to delete the other seeds
    ...
    if os.path.exists(out):
        try:
            ...
            print(f"  warning: {out} unparseable, starting fresh")
